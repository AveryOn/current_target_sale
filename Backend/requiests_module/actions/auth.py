...
def authenticate_service_person(db: Session, username: str, password: str, UUID: str, KEY_ACCESS: str) -> user.ServicePerson:
    if(KEY_ACCESS == MANAGER_KEY or KEY_ACCESS == OWNER_KEY):
        try:
...
# ast.literal_eval который переводит структуры данных (в данном случае dict) со строки в их нативный формат (в формат dict)
